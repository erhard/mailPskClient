...
//            isBulletin = false;
    ...
    private String analyze(String inString) {

        Main.logFile.writeToLog("Entering analyze.");
...
//            System.out.println("Nullstring in analyze");
        ...
        // check if bulletin
        Pattern pc = Pattern.compile("ZCZC");
        Matcher mc = pc.matcher(inString);
        if (mc.find()) {
            Main.Bulletinmode = true;
            Main.BulletinResetTimerToggle = true;
        ...
        if (!Main.Connected) {
            Pattern pf = Pattern.compile("ZFZF");
            Matcher mf = pf.matcher(inString);
            if (mf.find()) {
                // reinitialize fleet Decoder
                Main.fleetDecoder = new DecodeFleetCode();
                Main.logFile.writeToLog("Fleet Decoder reinitialized.");
                // --------------------------
                Main.IACmode = true;
                Main.logFile.writeToLog("IAC mode set to true.");
                //                       System.out.println("Starting IAC mode");
                //                      return "";
            }
        }

        //check if NNNN
        Pattern pn = Pattern.compile("NNNN");
        Matcher mn = pn.matcher(inString);
        if (mn.find() & Main.Bulletinmode) {
            //System.out.println("analyze NNNN with Bulletionmode.");

            Main.bulletinParser.inject("NNNN\n");
        ...
        if (Main.Bulletinmode) {
            //System.out.println("reinjecting Blockline unadjusted (Bulletinmode).");
            return inString;
        }
        if (Main.IACmode) {
            Main.logFile.writeToLog("IACMode in analyze, pull string through.");
            return inString;
        }
...
//00uPI4TUE:26 PA0R>PSKAPR*::PA0R     :ack06
                    ...
                    Pattern pd = Pattern.compile("^(.*):(\\d+)\\s(\\S+)\\s(.*)$");
